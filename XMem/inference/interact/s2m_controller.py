import torch
import numpy as np
from .s2m.s2m_network import deeplabv3plus_resnet50 as S2M
...
class S2MController:
    ...
    def interact(self, image, prev_mask, scr_mask):
        ...
        h, w = image.shape[-2:]
        unaggre_mask = torch.zeros(
            (self.num_objects, h, w), dtype=torch.float32, device=image.device
        )

        for ki in range(1, self.num_objects + 1):
            p_srb = (scr_mask == ki).astype(np.uint8)
            n_srb = ((scr_mask != ki) * (scr_mask != self.ignore_class)).astype(
                np.uint8
            )

            Rs = (
                torch.from_numpy(np.stack([p_srb, n_srb], 0))
                .unsqueeze(0)
                .float()
                .to(image.device)
            )
