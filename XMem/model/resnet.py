...
def resnet18(pretrained=True, extra_dim=0, model_path=None):
    ...
    if pretrained:
        if model_path is None:
            ...
            assert type(model_path) == str and model_path.endswith(
                ".pth"
            ), "model_path must be a string ending with .pth"
            load_weights_add_extra_dim(model, torch.load(model_path), extra_dim)

    return model


def resnet50(pretrained=True, extra_dim=0, model_path=None):
